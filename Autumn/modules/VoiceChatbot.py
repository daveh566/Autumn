...
from Autumn.services.pyrogram import pbot as LYCIA
...
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Autumn AI Voice Chatbot By @famouskaykay")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("Autumn Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Autumn&ownername=Autumn&user=1"
        )
        chatbot = L["message"]
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @famouskaykay...")

...
from Autumn.function.pluginhelpers import get_text, progress
from Autumn.services.pyrogram import pbot as Client
...
async def ytmusic(client, message: Message):
    ...
    if dl_limit >= 4:
        await message.reply_text(
            "Autumn's server busy due to too many downloads, try again after sometime."
        )
        return
...
async def jssong(_, message):
    ...
    if dl_limit >= 3:
        await message.reply_text(
            "Autumns's server busy due to too many downloads, try again after sometime."
        )
        return
    if is_downloading:
        await message.reply_text(
